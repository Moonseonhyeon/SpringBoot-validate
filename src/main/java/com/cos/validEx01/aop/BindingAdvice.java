...
public class BindingAdvice {
	...
	public void test1() {
		System.out.println("BindController에 오신것을 환영합니다. Before");

	}

	@After("execution(* com.cos.validEx01.test.BindControllerTest.*(..))")	
	public void test2() {
		System.out.println("BindControllerTest를 이용해 주셔서 감사합니다. After");
	}
	...
	public Object validationHandler(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
		// joinPoint(메서드의 핵심로직 context)를 proxy로 데리고 온거임.																								
		...
		// joinPoint에 접근하는 이유는 bindingResult에 접근하기 위해서
		...
		// 이후 처리를 위해서는 request에 접근하고 싶은건데 joinPoint안에 들어가서
		//지지고 볶고 난 후의 request를 내가 눈으로 볼 수 있어야 해요 .
